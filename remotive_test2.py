...
from openpyxl.styles import Font, PatternFill, GradientFill, Alignment, Border, Side # importing styles for formatting the excel output @jenny
...
OUTPUT_PATH = "week_4/remotive_jobs_scored2.xlsx"
# ----------------------------
# HELPER FUNCTIONS
# ----------------------------
...
for col, header in enumerate(headers, 1):
    ...
    cell.font = Font(name = "Calibri", bold=True, color ="FFFFFF", size = 12)  # Make header bold
    cell.fill = GradientFill(
        type="linear",
        degree=90,             # angle of gradient
        stop=["FF0097A7", "FF004D40"]  # start and end color
    )  # Teal background. This is working but not in the VSCode preview.

    cell.alignment = Alignment(horizontal="center", vertical = "center", wrap_text = True)  # Center align header text
    ws_jobs.freeze_panes = "A2"  # Freeze header row
    ws_jobs.auto_filter.ref = f"A1:L{len(filtered_jobs)+1}"  # Add autofilter to header row
    ws_jobs.row_dimensions[1].height = 20  # Increase header row height for better readability

# Define alternating row colors @jenny
ROW_COLORS = ["FFFFFF", "FFECEFF1"]  # white and gray team alternating.
...
for row, job in enumerate(filtered_jobs, 2):
    ...
    if rank == 1:
        row_bg = "FFFFD700"  # Gold for rank 1
    elif rank == 2:
        row_bg = "FFC0C0C0"  # Silver for rank 2
    elif rank == 3:
        row_bg = "FFCD7F32"  # Bronze for rank 3
    elif rank <= 10:
        row_bg = "FFE0F7FA"  # Light Teal for ranks 4-10
    else:
        row_bg = "FFECEFF1" if row % 2 == 0 else "FFFFFFFF" # alternating white/gray for the rest of the list
    
    # Apply fill to all 5 columns in that row @jenny
    for col in range(1, len(headers) +1):
        cell = ws_jobs.cell(row=row, column=col)
        cell.fill = PatternFill(start_color=row_bg, end_color=row_bg, fill_type="solid")

    # # Adding color @jenny
    # bg = ROW_COLORS[row % 2]  # Alternate row colors
    # for col in range(1, len(headers) + 1):
    #     cell = ws_jobs.cell(row=row, column=col)
    #     cell.fill = PatternFill(fill_type="solid", start_color=bg, end_color=bg)
    
    # Define salary cell font @jenny
    salary_cell = ws_jobs.cell(row=row, column = 6) # this is the salary column

    if job["salary"]:
        salary_cell.font = Font(bold = True, color="FF00695C")  # Deep Teal font for salary presence
